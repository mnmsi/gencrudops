...
namespace Mnmsi\GenCrudOps\Services;
...
class MakeViewsService extends IOServices
{
    ...
    public function findAndReplaceController($columns, $controllerStub, $namingConvention)
    {
        $cols = '';
    ...
    public function findAndReplaceIndexView($columns, $templateViewsDirectory, $namingConvention, $separateStyleAccordingToActions)
    {
        $thIndex = $indexView = '';
        foreach ($columns as $column) {
            $type   = explode(':', trim($column));
        ...
        $indexStub = File::get($this->pathsAndNamespacesService->getGencrudopsViewsStubCustom($templateViewsDirectory) . DIRECTORY_SEPARATOR . 'index.stub');
    ...
    public function findAndReplaceCreateView($columns, $templateViewsDirectory, $namingConvention, $separateStyleAccordingToActions)
    {
        $formCreate = '';
        foreach ($columns as $column) {
            $type     = explode(':', trim($column));
            ...
            $formCreate .= str_repeat("\t", 3) . '<label for="' . trim($column) . '" class="form-label">' . ucfirst(trim($column)) . '</label>' . "\n";
            $formCreate .= str_repeat("\t", 3) . '<input type="' . $typeHtml . '" name="' . trim($column) . '" value="" class="form-control">' . "\n";

            $formCreate .= str_repeat("\t", 2) . '</div>' . "\n";
        }

        $createStub = File::get($this->pathsAndNamespacesService->getGencrudopsViewsStubCustom($templateViewsDirectory) . DIRECTORY_SEPARATOR . 'create.stub');
    ...
    public function findAndReplaceShowView($templateViewsDirectory, $namingConvention, $separateStyleAccordingToActions)
    {
        $showStub = File::get($this->pathsAndNamespacesService->getGencrudopsViewsStubCustom($templateViewsDirectory) . DIRECTORY_SEPARATOR . 'show.stub');
    ...
    public function findAndReplaceEditView($columns, $templateViewsDirectory, $namingConvention, $separateStyleAccordingToActions)
    {
        $formEdit = '';
        foreach ($columns as $column) {
            $type     = explode(':', trim($column));
            ...
            $formEdit .= str_repeat("\t", 3) . '<label for="' . trim($column) . '" class="form-label">' . ucfirst(trim($column)) . '</label>' . "\n";
            $formEdit .= str_repeat("\t", 3) . '<input type="' . $typeHtml . '" name="' . trim($column) . '" class="form-control" value="{{ DummyCreateVariableSing$->' . trim($column) . ' }}">' . "\n";
            $formEdit = str_replace('DummyCreateVariableSing$', '$' . $namingConvention['singular_low_name'], $formEdit);
            $formEdit .= str_repeat("\t", 2) . '</div>' . "\n";
        }

        $editStub = File::get($this->pathsAndNamespacesService->getGencrudopsViewsStubCustom($templateViewsDirectory) . DIRECTORY_SEPARATOR . 'edit.stub');
